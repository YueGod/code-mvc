  ...
  public Set<RegistryDescription> getRegistryClazz(Set<Class<?>> scannedClass) {
    ...
    for (Class<?> clazz : scannedClass) {
      if (isRegistry(clazz)) {
        ...
        RegistryDescription registryDescription = new RegistryDescription(clazz.getName(), clazz,
            registry.order());
        registryDescriptions.add(registryDescription);
      }
    }
    //按Order进行排序
    return registryDescriptions.stream()
        .sorted(Comparator.comparingInt(RegistryDescription::getOrder)).collect(
