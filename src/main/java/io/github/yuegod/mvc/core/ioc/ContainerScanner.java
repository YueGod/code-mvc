...
public class ContainerScanner {
     ...
     * 需要替换的符号
     */
    private static final char PACKAGE_PATH_REGEX = '.';

    /**
     * 通过全路径扫描class文件
     */
    public Set<Class<?>> findClassesByPackagePath(String packagePath) {
        //第一个class类的集合
        Set<Class<?>> classes = new HashSet<>();
        //是否循环迭代
        boolean recursive = true;
        //获取包的名字 并进行替换
        String packageDirName = packagePath.replace(PACKAGE_PATH_REGEX, '/');
    ...
    public void findAndAddClassesInPackageByFile(String packageName, String packagePath, final boolean recursive, Set<Class<?>> classes) {
        ...
        for (File file : dirfiles) {
            //如果是目录 则继续扫描
            if (file.isDirectory()) {
                findAndAddClassesInPackageByFile(packageName + PACKAGE_PATH_REGEX + file.getName(),
                ...
                try {
                    //添加到集合中去
                    classes.add(Class.forName(packageName + PACKAGE_PATH_REGEX + className));
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                }
            }
    ...
    public Set<Class<?>> scan(String packagePath) {
        return findClassesByPackagePath(packagePath);
    }
